# beat-em-up-research.md — Full Archive

> Archived version. Compressed operational copy at `beat-em-up-research.md`.
...
The beat 'em up combat loop is deceptively simple on the surface but contains significant depth when well-designed:

```
BASIC LOOP:
  Approach → Light Attack String (2-4 hits) → Combo Ender (knockdown/launch)
  ↓
  Grab → Throw / Slam / Pummel
  ↓
  Weapon Pickup → Modified attack string (more range/damage, breaks after use)
  ↓
  Special Move → Crowd clear / Combo extension (costs resource)

ADVANCED LOOP:
  Approach → Light string → Cancel into Special → Juggle → Wall bounce →
  Air combo → Slam → OTG pickup → Repeat
```

**Across reference games, the essential combat inputs are:**

| Input | Function | Present In |
|-------|----------|-----------|
...
Beat 'em ups use a **2.5D plane** (horizontal scrolling with vertical movement for depth). Crowd control is the hidden skill ceiling:

- **Juggling:** Launching enemies upward and keeping them airborne with timed hits. Creates combo extensions and prevents enemies from acting. (SoR4 elevated this to an art form.)
- **Crowd CC:** Moves that hit multiple enemies — spinning attacks, wide swings, thrown enemies colliding with others. Essential when surrounded.
- **Positioning:** The "dance" of the genre. Players constantly adjust vertical position to line up attacks and avoid being flanked. Expert players manipulate enemy positions by walking up/down to cluster them.
- **Knockdown management:** Deciding which enemies to knock down (removing them temporarily) vs. which to keep standing (for grab opportunities or combo fodder).

### 2.3 Enemy Design — The Vocabulary

Great beat 'em ups build a "vocabulary" of enemy types that players learn to read and counter:

| Enemy Type | Role | Counter Strategy | Examples |
|-----------|------|-----------------|----------|
| **Grunt** | Fodder, combo practice | Basic attacks | Foot Soldiers, Galsia |
| **Charger** | Punishes stationary play | Dodge/jump, then punish | Signal, Bull enemies |
| **Ranged** | Forces approach, area denial | Priority target, close distance | Gunners, knife throwers |
| **Shield/Guard** | Requires grab or back attack | Grab, jump attack, flank | Riot cops, shield knights |
| **Grappler** | Punishes proximity, reverses grabs | Hit-and-run, keep distance | Large brutes, wrestlers |
| **Aerial** | Controls vertical space | Anti-air attacks, timed jumps | Flying enemies, ninjas |
| **Summoner** | Creates urgency, priority target | Rush down immediately | Wizards, commanders |
| **Mini-boss** | Skill check, mid-level spike | Pattern recognition, patience | Named enemies, palette swaps |
| **Boss** | Level climax, pattern puzzle | Learn tells, punish windows | Unique per stage |

**The golden rule:** Never introduce more than one new enemy type per encounter. Let players learn the "word" before building "sentences."

### 2.4 Level Design

Beat 'em up levels follow a predictable but effective structure:

```
LEVEL STRUCTURE:
  Walk-in (mood setting) → Encounter 1 (easy warm-up) →
  Scroll → Encounter 2 (introduce new element) →
  Scroll → Encounter 3 (difficulty spike / mini-boss) →
  Scroll → Encounter 4 (mix of learned elements) →
  Camera Lock → BOSS FIGHT → Victory / Score Screen
```

**Key level design elements:**
- **Camera locks:** The screen stops scrolling and enemies spawn. Creates arena encounters. Frequency matters — too many feels like a gauntlet; too few makes traversal boring.
- **Environmental hazards:** Pits, fire, crushers, moving vehicles. Best when they affect enemies too (throwing a goon into a pit is peak satisfaction).
- **Destructible objects:** Crates, barrels, phone booths, newspaper stands. Contain health, weapons, score items. Breaking things feels good and rewards exploration.
- **Pacing variation:** The best games alternate intensity. A tough arena fight should be followed by a breezy walk-and-smash section. SoR4's elevator level is a masterclass in escalating tension.
- **Set pieces:** Unique moments that break the formula — riding a vehicle, a chase sequence, a scrolling hazard. Turtles in Time's surfboard level, SoR2's pirate ship.

### 2.5 Progression Systems

Progression has evolved dramatically across the genre's history:
...
Co-op is the genre's killer feature:
...
### 2.7 Audio/Visual Feedback ("Game Feel")

The invisible layer that makes combat satisfying:

- **Hitlag / Hitstop:** A 2–4 frame pause on impact. Makes hits feel heavy. SoR4 uses variable hitstop — light attacks pause 2 frames, heavy attacks pause 4+.
- **Screen shake:** Subtle camera shake on big hits. Should scale with impact force.
- **Hit flash:** Enemy sprites flash white on hit. Universal across the genre.
- **Impact particles:** Sparks, stars, dust clouds on contact. Communicates damage visually.
- **Sound design:** The crunch of a punch landing, the crack of a bat connecting, the whoosh of a whiffed attack. Audio is 50% of "game feel."
- **Knockback animation:** Enemies should visibly react to each hit with appropriate force. A jab nudges; a heavy punch sends them flying.
- **Kill effects:** Extra dramatic death animations for the last enemy in an encounter. Slow-mo, extra particles, dramatic sound.

---

## 3. What Makes Them FUN? — The "Juice" Factors

### 3.1 Satisfying Hit Feedback (Weight & Impact)

The #1 differentiator between a good and great beat 'em up. Every attack must communicate:
- **"I hit something"** — visual confirmation (flash, particles)
...
The best encounters are puzzles: "There's a shielded enemy, a ranged attacker, and three grunts. Who do I prioritize? What tool do I use?" This emergent decision-making is what separates combat from button mashing.

**Encounter design formula:**
- **Encounters 1–3:** Single enemy type, learn the matchup
- **Encounters 4–6:** Two types combined, learn prioritization
- **Encounters 7+:** Three+ types, full tactical thinking required
- **Boss:** Unique pattern, tests all learned skills

### 3.3 Power Fantasy

Players should feel STRONG. The ratio should be roughly:
- 70% of the time: player is dominant, mowing through enemies, feeling like a badass
- 20% of the time: player is challenged, needing to play smart
- 10% of the time: player is overwhelmed, creating dramatic tension

This ratio is critical. Too much challenge and the game is exhausting. Too little and it's boring. Castle Crashers skews toward power fantasy (80/15/5); SoR4 on higher difficulties skews toward challenge (50/30/20).

### 3.4 Risk/Reward Tension

Every great beat 'em up creates moments where the player must choose:
- "Do I use my special (costs health) to clear this crowd, or try to fight through?" (SoR2)
- "Do I taunt for extra super meter while enemies are nearby?" (Shredder's Revenge)
- "Do I go for the extended juggle combo for more points, or play it safe?" (SoR4)
- "Do I grab this enemy for a throw, risking another enemy hitting me from behind?" (All games)

These micro-decisions, made every few seconds, create engagement that pure button-mashing cannot.

### 3.5 Combat Rhythm & Flow

Great beat 'em up combat has a rhythm — attack, pause, reposition, attack. This cadence should feel like a dance:
- **Attack windows:** After an enemy's attack whiffs, there's a punish window. Recognizing and exploiting these creates flow.
- **Combo rhythm:** Attack strings should have a musical timing — tap-tap-TAP-pause-TAP. The rhythm varies by character (Guy is staccato; Haggar is legato).
- **Encounter pacing:** Waves of enemies should ebb and flow. A burst of 6 enemies, thin to 2, then another burst of 4. Constant pressure is exhausting; rhythmic pressure is engaging.

### 3.6 Environmental Interaction

Using the environment makes players feel clever:
- Throwing enemies into pits, off ledges, into hazards
- Picking up and using scenery as weapons (trash cans, signs, benches)
...
## 4. Modern Innovations Worth Adopting (2018–2024)

### 4.1 Combo Scoring / Style Meters

**What it is:** A visible score multiplier that increases with consecutive hits and resets if you take damage or let the combo timer expire.

**Games using it:** SoR4, River City Girls 2, Fight'N Rage
...
**What it is:** Enemies behave as physics objects when launched, bouncing off walls, floors, and ceilings. Players can extend combos by timing hits during bounces.

**Games using it:** SoR4, Dragon's Crown, Fight'N Rage
...
**What it is:** A dedicated taunt button that builds your super meter faster but leaves you vulnerable during the animation.

**Games using it:** Shredder's Revenge, River City Girls
...
**What it is:** A meter that fills through combat and, when full, allows a powerful screen-clearing or combo-extending super move.

**Games using it:** Shredder's Revenge, Castle Crashers, River City Girls
...
**What it is:** Per-level and full-game leaderboards tracking score, time, and combo performance. Some games include replay recording.
...
**What it is:** Persistent upgrades that carry between play sessions — new moves, stat boosts, cosmetics.

**Games using it:** Scott Pilgrim, Castle Crashers, River City Girls
...
**What it is:** A dedicated dodge/roll that grants brief invincibility during the animation, allowing players to avoid damage through timing.

**Games using it:** SoR4, Shredder's Revenge, River City Girls
...
**What it is:** Options to simplify gameplay for less experienced players — auto-combos, reduced damage, extra lives, one-button specials.

**Games using it:** Shredder's Revenge, SoR4
...
5. ✅ Enemy type vocabulary (grunts, chargers, ranged, shields, bosses)
6. ✅ Environmental interaction (destructibles, throwable objects, hazards)
7. ✅ Multiplayer co-op (at minimum 2-player, ideally 4-player)
