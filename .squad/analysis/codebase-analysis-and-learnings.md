# SimpsonsKong — Full Codebase Analysis & Learnings
...
| QW-7 | **Simpsons pickup sounds** (AAA-A6) | `audio.js` | Add `playPickupDonut()`, `playPickupCola()` methods (3 oscillators each). Wire in gameplay.js destructible drop handler. | ★☆☆ Low |
...
| QW-21 | **Dynamic crowd reactions on combo** (AAA-A3 partial) | Partially done | Woohoo on 5+ combo exists (gameplay.js line 297). D'oh on damage (line 349). Missing: crowd cheering audio layer. | ★☆☆ Low |
...
| ME-9 | **Dash attack** (AAA-C7) | `player.js`, `combat.js` | 2h | New state 'dash_attack' — double-tap forward + attack. Homer runs forward 150px with belly hitbox. Similar to belly_bump but from neutral (no combo req). | ★★☆ Medium | Dodge (done) |
| ME-10 | **Taunt mechanic** (AAA-C5) | `player.js`, `input.js`, `audio.js` | 2h | New state 'taunt' with dedicated key. 1s vulnerability. Builds style meter. Homer eats invisible donut. Cancellable on hit. | ★★☆ Medium | Style meter (ME-4) |
| ME-11 | **Super move meter + activation** (AAA-C6) | `player.js`, `hud.js`, `combat.js`, `vfx.js` | 3-4h | New meter filled by combat+taunts. When full, special input triggers Donut Rage (temporary berserk). Camera zoom on activation. New HUD element. | ★★★ High | Taunt (ME-10), Style (ME-4) |
...
| ME-14 | **Character voice barks** (AAA-A1) | `audio.js` | 2-3h | Procedural voice synthesis: D'oh (descending tone + vowel filter), Woohoo, Mmm-donuts. Audio.js already has `playDoh()`, `playWoohoo()` stubs. Enhance with formant filters. | ★★☆ Medium | None |
...
| ME-25 | **Mini-boss encounters** (EX-T8) | `data/levels.js`, `ai.js` | 2h | Create mini-boss variants (e.g., tough_nelson) with unique AI patterns. Data-driven from levels.js. | ★★☆ Medium | Behavior upgrade (ME-21) |
...
| ME-28 | **Homer walk cycle animation** (P1-9) | `player.js` | 3h | Replace `walkBob` sine wave (line 603) with proper multi-frame walk: legs alternate, arms swing, body sway. All in player.js render method. 400 LOC of Canvas drawing to modify. | ★★★ High — visual quality | AnimationController (ME-2) |
| ME-29 | **Homer attack animations** (P1-10) | `player.js` | 3h | Replace static arm positions with multi-frame anticipation → active → recovery. Wind-up frames, follow-through. Currently just arm angle changes. | ★★★ High — combat feel | AnimationController (ME-2) |
...
| ME-32 | **Background landmark expansion** (vis-mod §3.2) | `background.js` | 2-3h | background.js already has Kwik-E-Mart, Moe's, Android's Dungeon, Elementary, Lard Lad, Leftorium, Jebediah statue. Add: Krusty Burger, Flanders house, Simpson house, town hall. | ★☆☆ Low | None |
