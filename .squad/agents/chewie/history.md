...
### Flora PR Review Fixes (2026-03-11)
- **PR #18 — UI Memory Leaks:** Added destroy() methods to all UI components (HUD, ToolBar, PlantInfo, DaySummary, SeedInventory, PauseMenu). Pattern: removeAllListeners() on interactive containers, destroy() on all PixiJS text/graphics objects, destroy({children: true}) on child containers, removeChildren() on self, then super.destroy(). Event listeners on ToolBar buttons and SeedInventory panel were the primary leak vectors.
- **PR #19 — Audio Volume Persistence:** Added localStorage persistence for volume preferences. Separated user volume preferences from mute state — unmuting now restores user-adjusted volumes instead of resetting to hardcoded AUDIO.VOLUMES constants. Pattern: volumePreferences object holds user levels, _loadPreferences() at init reads from 'flora:audio:preferences', _savePreferences() writes after every volume/mute change. Graceful fallback with try/catch if localStorage unavailable.
- **Lesson:** PixiJS v8 Container lifecycle requires explicit cleanup. EventMode 'static' + event handlers = memory leak if not removed. Always call removeAllListeners() before destroy(). For volume systems: mute state ≠ volume preference. Store last known volume so unmute restores user intent, not factory defaults.
