...
- **Hero design proposal workflow:** Generated 3 design variations per character at 1024×1024 for founder approval before committing to animation frames. Variations focused on pose, outfit details (shoulder pad placement, gi closure style), and energy effect presentation. Contact sheets (3-up side-by-side) make comparison easy. Files saved to `games/ashfall/assets/poc/designs/`.
- **FLUX 2 Pro costume detail control:** Specifying hex color codes inline with garment descriptions (e.g., "near-black sleeveless tank top (#331F1A)") gives FLUX better color guidance. Barefoot characters need explicit "NO shoes NO boots" to prevent the model from adding footwear by default.
- **PoC Sprite Background Removal (2026-03-12):** Batch-processed all 30 character sprites in `games/ashfall/assets/poc/` using Python `rembg` (u2net model) to remove inconsistent opaque backgrounds (white/brown/grey) and add alpha transparency. Files processed in-place: kael_hero, rhena_hero, 8 idle, 8 walk, 12 lp frames. Excluded embergrounds_bg.png and title_screen.png (full backgrounds). All output verified with alpha range 0-255. `rembg` with `[cpu]` extra is ~176 MB model download on first run; subsequent runs are instant from cache at `~/.u2net/u2net.onnx`.
- **FLUX API Art PoC (2026-03-11):**Successfully generated 32 images via Azure FLUX APIs. Used FLUX 2 Pro for 4 hero/scene images (kael_hero, rhena_hero, embergrounds_bg, title_screen) and FLUX 1 Kontext Pro for 28 sprite frames (8 idle, 8 walk, 12 attack_lp) using kael_hero.png as input_image reference. All output in `games/ashfall/assets/poc/`.
- **FLUX content filter workaround:** Kontext Pro rejects prompts with direct combat language ("punch," "jab," "fist cocked," "strike," "impact"). Rewrite using martial arts kata language ("arm extending forward," "controlled motion," "martial arts form") to pass content filters while maintaining pose clarity.
- **FLUX Kontext Pro consistency strategy:** Lock seed per animation sequence (e.g., seed=1001 for all idle frames, seed=2001 for walk). Combined with input_image reference from hero frame, this produces usable frame-to-frame consistency. Not perfect temporal coherence but sufficient for PoC validation.
- **FLUX output characteristics:** Raw FLUX PNGs average ~680 KB per 512×512 sprite (much larger than expected 50-100 KB). Backgrounds are not transparent — post-processing pipeline needed for: background removal → palette enforcement → compression → sprite sheet assembly.
- **FLUX 2 Pro rate limit management:** 4 req/min hard limit. Use 15s sleep between calls. Kontext Pro at 30/min is much more generous — 3s gaps sufficient. Token expires ~1h; refresh for long batch sessions.
- **Sprint 2 Phase 1 sprite detail upgrade (PR #147):** Upgraded both character sprites from placeholder-quality to 90s arcade-quality procedural renderings. Added 11 new palette color keys per palette (skin_shadow/highlight, hair_highlight, eyebrow, nose_shadow, eye_white, gi/muscle detail colors). Created detailed head base helpers (_draw_kael_head_base, _draw_rhena_head_base) with neck, jaw shadow, ear, volumetric hair with highlight strands, nose bridge, and eyebrows. All head expression helpers now use iris+sclera eyes and angled eyebrow lines for much more expressive faces.
- **Shared detail helper pattern:** Created reusable body detail methods (_draw_gi_torso/_draw_tank_torso, _draw_detailed_belt, _draw_detailed_arm, _draw_detailed_leg, _draw_detailed_boot, _draw_shoulder_joint) that apply consistently across all poses using them. This is much more maintainable than adding inline detail to every pose individually — one helper upgrade cascades to all calling poses.
- **Palette-driven shading system:** Adding shadow/highlight color variants to palettes (skin_shadow, gi_shadow, etc.) rather than computing them at draw time ensures consistent visual quality and lets P2 alternate palettes get properly tuned shading colors. Dictionary keys must exist in BOTH P1 and P2 palettes or the Variant lookup will crash.
- **Head helper cascade strategy:** Since ~40 of 47 poses call shared head helpers (_draw_head_calm, _draw_head_focused, etc.), upgrading the head base rendering automatically improves the visual quality of nearly every animation state without touching individual pose functions. Highest-ROI visual upgrade path.
- **Torso shading technique:** Drawing two overlay polygons (shadow on back side, highlight on front side) over the base torso polygon creates convincing depth with just 3 draw_colored_polygon calls. Shadow is ~35% width from left edge, highlight is ~3px from right edge.
- **Ember VFX layering:** Using 3-5 concentric circles with decreasing alpha (0.08 → 0.15 → 0.5 → bright core) creates much more convincing energy effects than the original 1-2 layers. Motion trail particles (small circles with decreasing alpha trailing behind attack direction) add perceived speed.
- **Character-specific muscle rendering:** Kael uses gi_fold lines (fabric detail) while Rhena uses muscle_line colors (visible musculature through tank top). Same drawing technique (line overlays on torso polygon) but different visual meaning per character archetype.
...
## Ashfall Sprint 1 Character Sprite Completion (2026-03-09)

**Project:** Ashfall — 1v1 fighting game in Godot 4  
**Role:** Character Artist  
**Status:** Issues #99, #100 COMPLETED — PR #115 merged

### Issue #99 — Kael Final Animation Poses (6 poses)
...
### Issue #100 — Rhena Final Animation Poses (6 poses)
...
**Total Animation Poses:**
- **Before Sprint 1:** 41 poses per character (idle, crouch, jump phases, dash/backdash, 3 standing kicks, 6 crouching attacks, 6 jump attacks, block variants, throws, 4 specials, super)
- **After Sprint 1:** 47 poses per character (+6 new poses = complete GDD spec)

### Key Decisions

**Personality Differentiation:**
- Kael's new poses are controlled and composed even in adversity (failed grab = disappointment, hit = braced, knockdown = accepting)
- Rhena's new poses are explosive and chaotic (failed grab = angry, hit = screaming, knockdown = ragdoll tumble)
...
- **SpriteStateBridge pattern is reusable:** Context-aware pose selection (one state → multiple poses) becomes template for future games. Route by method check rather than state naming.
...
- **Team:** Character art complete for Sprint 2. All GDD animation specs delivered.
- **Pipeline:** Stage artist can now build backgrounds around finalized character bounding boxes (47 poses establish final silhouettes).
- **Cross-project:** SpriteStateBridge routing pattern ports to future Godot projects as reusable template.
- **Readiness:** Animation state machine now complete. Next phase is special move VFX language (Boba's domain).
