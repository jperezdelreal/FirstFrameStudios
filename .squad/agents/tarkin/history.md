...
### Flora Issue #7 — Hazard System: Pests & Weather Events (2026-03-11)
- **PR:** #20 (`squad/7-hazard-system`) — repo: jperezdelreal/flora
- **Hazard entity (src/entities/Hazard.ts):** Polymorphic entity with two hazard types — PEST (targets individual plants) and DROUGHT (affects entire garden). Pest data includes target plant ID, spawn day, state (SPAWNED/ACTIVE/REMOVED), and damage per day. Drought data includes start day, duration, days remaining, water need multiplier, and active flag. Entity methods: `updatePestState()`, `advanceDrought()`, type-checked accessors `getPestData()` and `getDroughtData()`.
- **Hazard config (src/config/hazards.ts):** Base configs for PEST (spawn window day 6-8, max 2 pests per event, 12 dmg/day, 30% resistance at >70% health) and DROUGHT (warning day 5, duration 2-3 days, 1.5x water need multiplier). Difficulty scaling function `getDifficultyScaling(seasonCount)` uses 0..1 ramp (season 1→9) like ComeRosquillas ghost AI, scaling pest spawn chance (0.5x→1.2x), drought intensity (1.0x→1.3x), and max simultaneous hazards (1→3).
- **HazardSystem (src/systems/HazardSystem.ts):** Day-based system coordinating with PlantSystem via `onDayAdvance(day)`. Tracks spawn windows (pest spawn checked on day 6-8, drought on day 5+), manages active hazards in Map. Provides `trySpawnPestOnPlant(plant)` for external pest spawning with resistance check, `applyPestDamage(plants)` to damage targeted plants, `getDroughtMultiplier()` for PlantSystem water need scaling. Pest removal via `removePest(pestId)` (player click action). Auto-cleanup of inactive hazards.
- **Core design principles:**
  - **Never instant-fail:** Pests damage over time (12 dmg/day), player can always remove them (1 action cost). Drought warning appears 1 day before event starts. Healthy plants (>70% health) have 30% chance to resist pest spawning.
  - **Player counterplay:** Pest removal is a click action (costs 1 action per day but solvable). Drought increases water needs but doesn't kill plants outright. Player can sacrifice pest-infested plants and compost them.
  - **Difficulty scaling without frustration:** Early seasons (1-3) spawn 1 pest with low damage, late seasons (7-9) spawn 2-3 pests with higher damage but player has unlocked better tools by then. Drought duration extends from 2 to 3-4 days but water need multiplier only goes from 1.5x to ~2x.
- **System coordination pattern:** HazardSystem is day-based (not frame-based) and relies on external day advance notification. PlantSystem calls `hazardSystem.onDayAdvance(currentDay)` after advancing plants. HazardSystem returns drought multiplier via `getDroughtMultiplier()` which PlantSystem applies to plant water needs. Pest damage applied via `applyPestDamage(plants)` which takes Plant array, finds targets by ID, and calls `plant.takeDamage()`.
- **TypeScript polymorphism without type guards:** Initial implementation used `this is Hazard & { data: PestData }` type guards but TypeScript complained about private property `data` creating impossible intersections. Solution: changed `isPest()` and `isDrought()` from type guards to simple boolean checks, kept data access via `getPestData()` / `getDroughtData()` with explicit `as` casts. Simpler and compiles cleanly.
- **Integration hooks for UI:** System exports clean read-only interfaces: `getDroughtInfo()` returns `{ active, daysRemaining, multiplier }` for weather UI, `getActivePests()` returns array of pest hazards for tile markers, `getPestAt(x, y)` for click detection, `getPestSpawnWindow()` and `getDroughtWarningDay()` for tutorial hints.
- **Key insight for cozy games:** Hazards in non-combat games work best as **resource management puzzles** rather than threats. Pests are "plants need attention" signals, not punishment. Drought is "plan your watering differently for 2 days", not "you lose". Resistance mechanics (healthy plants resist pests) reward good play without hard-gating. Difficulty scaling should increase complexity/cadence (more hazards, less time between) rather than lethality (higher damage, instant death).
