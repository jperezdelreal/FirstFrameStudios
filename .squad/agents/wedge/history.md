...
- **HUD Integration into Fight Scene:**
  - Instanced `fight_hud.tscn` directly in `fight_scene.tscn` as a child node `FightHUD` — preferred over runtime loading because the HUD is self-wiring (connects all 11 EventBus signals in its own `_ready()` via `_wire_signals()`).
  - Added `@onready var hud: CanvasLayer = $FightHUD` reference in `fight_scene.gd`.
  - Set P1/P2 name labels from `SceneManager.p1_character` / `p2_character` (uppercased) so HUD shows actual character names instead of "P1"/"P2".
